public class Exercise6 {
    ...
    public static void main(String[] args) {
        ...
        String aa = "(9+9*7)";
        if (checkIfThereAreClosures(aa) == true)
    ...
    public static boolean checkLegal(String expression) {
        ...
        for (int i = 0; i < expression.length() - 2; i++) {
            ...
            if ((a >= THE_VALUE_OF_ZERO && a <= THE_VALUE_OF_NINE) && CheckMarkType(b) == true && (c >= THE_VALUE_OF_ZERO && c <= THE_VALUE_OF_NINE)) {
                i = i + 1;
            } else {
                legal = false;
            }
    ...
    public static boolean CheckMarkType(char mark) {
        ...
        if (mark == THE_VALUE_OF_MULTIPLICATION || mark == THE_VALUE_OF_PLUS || mark == THE_VALUE_OF_MINUS || mark == THE_VALUE_OF_DIVISION || mark == THE_VALUE_OF_STRONG) {
            markType = true;
        }
        return markType;
    }
